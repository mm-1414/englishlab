...
if name:
    prompt = st.text_area("お題(過去問などどんな問題でも入力可能である。例:Do the benefits of online shopping outweigh the disadvantages?)")
    user_essay = st.text_area("あなたの英作文を入力してください。問題に合わせて何文字でも入力可能である。")

    if st.button("採点する"):
        if prompt and user_essay:
            with st.spinner("採点中である..."):
                # 採点プロンプト(である調+gpt-4-turbo)
                system_message = """
あなたは英語の採点官である。
...
3. 文法・語法・単語・表現に間違いや不適切な点があれば、ミス1つごとに次の順番で詳しく書くこと。
   【元の文のミス箇所(英語)】→【訂正文(英語)】→【なぜ間違いなのか(日本語で丁寧なである調で詳しく。ただし必要に応じて単語や例文レベルでは英語を自然に使ってよい)】
...
※説明はすべて日本語(丁寧なである調)で書くこと。ただし単語や例文レベルでは自然に英語を使ってもよい。
※訂正文と模範解答は英語で示すこと。
...
"""
            ...
            st.warning("お題と英作文の両方を入力する必要がある。")
else:
    st.warning("最初に名前を入力する必要がある。")
